...
type CreateWishlistRequest struct {
	...
	AttachmentIDs Int64Array `json:"attachment_ids,omitempty"`                 // 附件ID数组
}
...
type WishlistDTO struct {
	...
	ReminderDate *string              `json:"reminder_date,omitempty"` // 格式: "2006-01-02"
	Attachments  []AttachmentResponse `json:"attachments,omitempty"`   // 关联的附件
...
func WishlistFromModel(wishlist *models.Wishlist) WishlistDTO {
	...
	// 处理附件
	if len(wishlist.Attachments) > 0 {
		dto.Attachments = make([]AttachmentResponse, len(wishlist.Attachments))
		for i, attachment := range wishlist.Attachments {
			dto.Attachments[i] = AttachmentFromModel(&attachment)
		}
	}
